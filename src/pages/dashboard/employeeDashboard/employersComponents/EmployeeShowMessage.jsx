    ...
    api
      .sendRequestMessage(
        token,
        {
          ...
          recipient_id: data[0].employer_id,
        },
        id
      )
      .then((response) => {
  ...
  return (
    <DashboardContent>
      <div>
        ...
        <div className=" flex max-h-[50vh] flex-col overflow-auto bg-white p-2 shadow-md dark:bg-dark-900 dark:text-dark-900">
          {data.map((message) => (
            <div
              key={message.id}
              className={`${
                selfData.data.result.id === message.employer_id
                  ? 'self-end'
                  : 'self-start'
